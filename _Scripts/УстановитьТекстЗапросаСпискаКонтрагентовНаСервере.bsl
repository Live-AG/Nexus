//УстановитьТекстЗапросаСпискаКонтрагентовНаСервере()
// гэсИзменитьТекстЗапросаПодзадач(ТекстЗапроса);


ВЫБРАТЬ РАЗРЕШЕННЫЕ
	ДанныеВнутреннихДокументов.Документ КАК Документ,
	ВЫРАЗИТЬ(ДанныеВнутреннихДокументов.КонтрагентыДляСписков КАК СТРОКА(250)) КАК Контрагент
...
ВЫБРАТЬ
	...
	ВЫРАЗИТЬ(ДанныеВходящихДокументов.КонтрагентыДляСписков КАК СТРОКА(250))
ИЗ
	РегистрСведений.ДанныеВходящихДокументов КАК ДанныеВходящихДокументов
ОБЪЕДИНИТЬ ВСЕ
ВЫБРАТЬ
	ДанныеИсходящихДокументов.Документ,
	ВЫРАЗИТЬ(ДанныеИсходящихДокументов.КонтрагентыДляСписков КАК СТРОКА(250))
ИЗ
...
ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	ВременнаяТаблицаКонтрагентов.Контрагент КАК Контрагент,
	ВЫБОР
...
ИЗ
	Задача.ЗадачаИсполнителя КАК ЗадачаИсполнителя
		...
		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаКонтрагентов КАК ВременнаяТаблицаКонтрагентов
		ПО ПредметыЗадач.Предмет = ВременнаяТаблицаКонтрагентов.Документ
ГДЕ
	...
	И ИСТИНА В
			...
			ГДЕ
				...
				И ИсполнителиРолейИДелегаты.ИсполнительДелегат = &ТекущийПользователь
				И ИсполнителиРолейИДелегаты.ИмяОбластиДелегирования В ("", "ПроцессыИЗадачи"))
	И ВременнаяТаблицаКонтрагентов.Контрагент = &Контрагент
