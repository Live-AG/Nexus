...
Функция СвойстваИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Словарь) Экспорт //Nexus +Экспорт
	
	//Nexus
	Если Словарь = Неопределено Тогда
		Словарь = СловарьСвойствИнформационнойБазы();
	КонецЕсли;
...
//Nexus
Функция СловарьСвойствИнформационнойБазы()
	
	Результат = Новый Структура();
	
	Результат.Вставить("ИдентификаторБазы",						"infobase");
	Результат.Вставить("ИмяВКластере",							"name");
	Результат.Вставить("ТипСУБД",								"dbms");
	Результат.Вставить("СерверБД",								"db-server");
	Результат.Вставить("БазаДанных",							"db-name");
	Результат.Вставить("ПользовательСервераБД",					"db-user");  
	Результат.Вставить("ЗащищенноеСоединение",					"security-level");
	Результат.Вставить("РазрешитьВыдачуЛицензийСервером",		"license-distribution");
	Результат.Вставить("БлокировкаРегламентныхЗаданий",			"scheduled-jobs-deny");
	Результат.Вставить("БлокировкаНачалаСеансов",				"sessions-deny");
	Результат.Вставить("НачалоБлокировки",						"denied-from");
	Результат.Вставить("СообщениеБлокировки",					"denied-message");
	Результат.Вставить("ПараметрБлокировки",						"denied-parameter");
	Результат.Вставить("КонецБлокировки",						"denied-to");
	Результат.Вставить("КодРазрешения",							"permission-code");
	Результат.Вставить("ВнешнееУправлениеСеансами",				"external-session-manager-connection-string");
	Результат.Вставить("ИспользованиеВнешнегоУправления",			"external-session-manager-required");
	Результат.Вставить("ПрофильБезопасности",					"security-profile-name");
	Результат.Вставить("ПрофильБезопасностиБезопасногоРежима",	"safe-mode-security-profile-name");
...
Функция ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, Словарь = Неопределено, Фильтр = Неопределено, ТипыСвойств = Неопределено)
	...
	// Подставим в командную строку путь до утилиты Trac и адрес сервера ras.
