...
Функция ПолучитьПутьКФайлуПрограммы(ВерсияПлатформы) Экспорт
	...
	Если ПараметрыСистемы.ТипПлатформы = ТипПлатформы.Windows_x86 
		...
		ПутьКФайлуПрограммы = ?(ЗначениеЗаполнено(ВерсияПлатформы),
							"C:\Program Files\1cv8\" + ВерсияПлатформы + "\bin\1cv8.exe",
							"C:\Program Files\1cv8\common\1cestart.exe");
	Иначе
		ПутьКФайлуПрограммы = ?(ЗначениеЗаполнено(ВерсияПлатформы), 
							"/opt/1cv8/x86_64/" + ВерсияПлатформы + "/1cv8", 
...
Функция ПолучитьСтрокуСоединенияСБазой(ИнформационнаяБаза, РежимЗапускаПриложения = "DESIGNER") Экспорт
	...
	ПутьКФайлуПрограммы = ПолучитьПутьКФайлуПрограммы(ПараметрыКонтекста.ВерсияПлатформы);
