...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "РегистрацияИнформационныхБаз" Тогда
		ПодключитьОбработчикОжидания("ПроверитьТекущиеЗаданияКластеров", 1, Ложь);
	КонецЕсли;
...
Функция ЕстьАктивныеЗаданияДляКластеров()
	...
	//	
	//	Если ДлительныеОперации.ЗаданиеВыполнено(Выборка.ИдентификаторЗадания) Тогда
	//		//#REF КонтрольнаяОчистка текщих заданий
...
Процедура ТаблицаИнформационныхБазВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе 
		ТекущийСтатус = Элементы.ТаблицаИнформационныхБаз.ТекущиеДанные.Статус;
		Если ТекущийСтатус = ПредопределенноеЗначение("Перечисление.СтатусыИнформационнойБазы.ВПодготовке") Тогда
			ИмяФормыБазы = "ФормаЭлемента";
		Иначе
			ИмяФормыБазы = "ФормаПросмотра";
		КонецЕсли;
			
	    	ОткрытьФорму("Справочник.ИнформационныеБазы.Форма." + ИмяФормыБазы, 
