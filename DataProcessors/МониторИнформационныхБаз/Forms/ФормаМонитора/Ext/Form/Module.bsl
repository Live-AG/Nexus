...
Процедура ДеревоЭлементовКластераВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТипЗнч(ЭлементКластера) = Тип("СправочникСсылка.Кластеры1С") Тогда
		
		ОткрытьФорму("Справочник.Кластеры1С.Форма.ФормаЭлемента", Новый Структура("Ключ", ЭлементКластера)); 
		
	ИначеЕсли ТипЗнч(ЭлементКластера) = Тип("СправочникСсылка.ИнформационныеБазы") Тогда
		
		ОткрытьФорму("Справочник.ИнформационныеБазы.ФормаЭлемента", Новый Структура("Ключ", ЭлементКластера));
