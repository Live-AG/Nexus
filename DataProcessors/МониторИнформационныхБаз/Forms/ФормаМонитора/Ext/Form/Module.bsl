...
Процедура ПриОткрытии(Отказ)
	...
	АктивныФоновыеЗадания = ЕстьАктивныеЗаданияДляКластеров();  
...
Процедура ВыборИзбранногоИзбранное(Команда)
	...
	ТекущиеДанные = Элементы.ТаблицаИнформационныхБаз.ТекущиеДанные;
	
	Если ТекущиеДанные <> Неопределено Тогда 
		ТекущаяИнформационнаяБаза	= ТекущиеДанные.Ссылка; 
		ГруппаИзбранного			= СтруктураМенюИзбранного[Команда.Имя];	
		
		ОбработатьВыборИзбранногоНаСервере(ТекущаяИнформационнаяБаза, ГруппаИзбранного);
		ЗаполнитьОтображениеМенюИзбранное(ТекущаяИнформационнаяБаза);
	КонецЕсли;
	
КонецПроцедуры 

&НаСервере
Процедура ОбработатьВыборИзбранногоНаСервере(ИнформационнаяБаза, ГруппаИзбранного)
	
	МассивСтрокаИзбранного = ТаблицаСоответствияИзбранного.НайтиСтроки(Новый Структура("ИнформационнаяБаза", ИнформационнаяБаза));
	
	ДобавитьВИзбранное = Истина;
	Для Каждого СтрокаИзбранного Из МассивСтрокаИзбранного Цикл
		
		Если СтрокаИзбранного.ГруппаИзбранных = ГруппаИзбранного Тогда
			ДобавитьВИзбранное = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	МассивЗначений = Новый Массив;
	МассивЗначений.Добавить(ИнформационнаяБаза);
	
	Если ДобавитьВИзбранное Тогда
		ДобавитьЗначенияВИзбранное(ГруппаИзбранного, МассивЗначений);	
	Иначе
		УдалитьЗначенияИзИзбранного(ГруппаИзбранного, МассивЗначений);	
	КонецЕсли;
	
	ПолучитьМассивИзбранногоДляБазы();

КонецПроцедуры // ОбработатьВыборИзбранногоНаСервере()
...
Процедура УстановитьОтборПоТаблицеБаз()
	...
	Если ВидОтбора = "Кластеры" И Элементы.ТаблицаЭлементовКластера.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элементы.ТаблицаЭлементовКластера.ТекущиеДанные.ИндексСостояния = 4 Тогда
	...
	ИначеЕсли ВидОтбора = "ВсеБазы" И Элементы.ТаблицаИзбранного.ТекущиеДанные <> Неопределено Тогда  
...
Процедура ПослеЗавершенияДлительногоЗадания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если Элементы.ТаблицаЭлементовКластера.ТекущиеДанные <> Неопределено
			И Элементы.ТаблицаЭлементовКластера.ТекущиеДанные.ИндексСостояния = 4 Тогда 
			Элементы.СтраницыИнформационныхБаз.ТекущаяСтраница = Элементы.СтраницаОперацияОжидания;
		Иначе 
			Элементы.СтраницыИнформационныхБаз.ТекущаяСтраница = Элементы.СтраницаИнформационныеБазы;
		КонецЕсли;
...
&НаКлиенте
Процедура КомандаИнформационныеБазы(Команда) 
...
Процедура ОтобразитьТекущийОтбор()
	...
	ИначеЕсли ВидОтбора = "ВсеБазы" Тогда
		Элементы.ГруппаКнопкиДобавить.ТекущаяСтраница = Элементы.ГруппаВсеБазы;
		
	КонецЕсли;
	
	Элементы.ГруппаВертикальноеРазделение.Видимость = ВидОтбора <> "ВсеБазы";
	
	Элементы.ТаблицаИнформационныхБазПроект.Видимость	= ВидОтбора <> "Проекты";
	Элементы.ТаблицаИнформационныхБазВладелец.Видимость	= ВидОтбора <> "Кластеры";
	
	Элементы.КомандаКластеры.Доступность			= ВидОтбора <> "Кластеры";
	Элементы.КомандаПроекты.Доступность				= ВидОтбора <> "Проекты";
	Элементы.КомандаИзбранное.Доступность			= ВидОтбора <> "Избранное";
	Элементы.КомандаИнформационныеБазы.Доступность	= ВидОтбора <> "ВсеБазы";
...
&НаКлиенте
Процедура ТаблицаЭлементовКластераПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
...
&НаКлиенте
Процедура ОбновитьРегистрациюБаз(Команда)
...
Процедура ТаблицаИнформационныхБазНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
 	...
 	Выполнение = (ВидОтбора = "Избранное") Или (ВидОтбора = "Кластеры");
...
&НаКлиенте
Процедура ТаблицаЭлементовКластераПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	
	СтандартнаяОбработка = Ложь;
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("ИнформационнаяБаза", ПараметрыПеретаскивания.Значение); 
	СтруктураПараметров.Вставить("Кластер1С", Строка);
	
	ДополнительныеПараметры = Новый Структура;
	
	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ОбработкаПереносаБазы", 
																			ЭтотОбъект, 
																			ДополнительныеПараметры); 
	
	ОткрытьФорму("Обработка.ОбработчикиСтандартныхСценариев.Форма.ПереносБазыВДругойКластер",
					СтруктураПараметров, 
					ЭтотОбъект, 
					ЭтотОбъект, 
					, 
...
Процедура ЗапуститьПредприятие(Команда)
	...
	КонтекстВыполнения.Вставить("ИнформационнаяБаза", ТекущаяСтрокаБазы.Ссылка);
...
&НаКлиенте
Процедура ОбработкаПереносаБазы(Результат, ДополнительныеПараметры) Экспорт
