...
Процедура ПолучитьГрафикПоказателейАктивности(ИнформационнаяБаза, ОбластьТабличногоДокумента, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СхемаКомпоновкиДанных = ПолучитьМакет("МакетКомпановкиДиаграммыПоказателей");
...
Функция ПолучитьТаблицуПользователей(ИнформационнаяБаза) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	МАКСИМУМ(ПользователиИБ.Ссылка) КАК ПользовательИБ,
	               |	ПользователиИБ.Наименование КАК ИмяПользователя
	               |ПОМЕСТИТЬ ВТ_ВсеПользователи
	               |ИЗ
	               |	Справочник.ПользователиИБ КАК ПользователиИБ
	               |ГДЕ
	               |	НЕ ПользователиИБ.ПометкаУдаления
	               |	И ПользователиИБ.Активный
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ПользователиИБ.Наименование
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	ИмяПользователя
	               |;
	               ...
	               |ПОМЕСТИТЬ ВТ_СеансыИнформационнойБазы
	               |ИЗ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_СеансыИнформационнойБазы.IPАдресКлиента КАК IPАдресКлиента,
	               |	ВТ_СеансыИнформационнойБазы.ВремяНачала КАК ВремяНачала,
	               |	ВТ_СеансыИнформационнойБазы.ВремяПоследнейАктивности КАК ВремяПоследнейАктивности,
	               |	ВТ_СеансыИнформационнойБазы.ИдентификаторИнформационнойБазы КАК ИдентификаторИнформационнойБазы,
	               |	ВТ_СеансыИнформационнойБазы.ИдентификаторПроцесса КАК ИдентификаторПроцесса,
	               |	ВТ_СеансыИнформационнойБазы.ИдентификаторСеанса КАК ИдентификаторСеанса,
	               |	ВТ_СеансыИнформационнойБазы.ИдентификаторСоединения КАК ИдентификаторСоединения,
	               |	ВТ_СеансыИнформационнойБазы.ИмяКомпьютера КАК ИмяКомпьютера,
	               |	ВТ_СеансыИнформационнойБазы.ИмяПользователя КАК ИмяПользователя,
	               |	ВТ_СеансыИнформационнойБазы.ИмяПриложения КАК ИмяПриложения,
	               |	ВТ_СеансыИнформационнойБазы.ИмяТекущегоСервиса КАК ИмяТекущегоСервиса,
	               |	ВТ_СеансыИнформационнойБазы.Лицензии КАК Лицензии,
	               |	ВТ_СеансыИнформационнойБазы.СоединениеСУБД КАК СоединениеСУБД,
	               |	ВТ_СеансыИнформационнойБазы.СпящийСеанс КАК СпящийСеанс,
	               |	ВТ_ВсеПользователи.ПользовательИБ КАК ПользовательИБ
	               |ПОМЕСТИТЬ ВТ_СеансыПользователей
	               |ИЗ
	               |	ВТ_СеансыИнформационнойБазы КАК ВТ_СеансыИнформационнойБазы
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ВсеПользователи КАК ВТ_ВсеПользователи
	               |		ПО ВТ_СеансыИнформационнойБазы.ИмяПользователя = ВТ_ВсеПользователи.ИмяПользователя
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	ИмяПользователя
	               |;
	               ...
	               |	ЕСТЬNULL(ВТ_ПользователиИБ.ПользовательИБ, ВТ_СеансыПользователей.ПользовательИБ) КАК ПользовательИБ
	               |ИЗ
