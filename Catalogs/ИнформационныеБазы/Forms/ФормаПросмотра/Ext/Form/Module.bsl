...
Процедура ОтобразитьВремяОкончанияБлокировки()
	...
	Если ВремяОкончанияБлокировки > ТекущаяДата() И БлокировкаНачалаСеансов Тогда
		Элементы.БлокировкаНачалаСеансов.Заголовок = " до " + Формат(ВремяОкончанияБлокировки, "ДФ='dd.MM.yy (HH:mm)'");
	Иначе
		Элементы.БлокировкаНачалаСеансов.Заголовок = " ";
	КонецЕсли;	
