...
Процедура УстановитьБлокировкуРегламентныхЗаданийНаСервере(ИнформационнаяБаза, Заблокировать)
	...
	АдминистрированиеЭлементовКластера.УстановитьБлокировкуРегламентныхЗаданий(ИнформационнаяБаза, Заблокировать);					
...
Функция НачатьПолучениеСвойствБазы()
	...
	Результат = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
												"АдминистрированиеЭлементовКластера.ПолучитьСвойстваИнформационнойБазы",
												Объект.Ссылка);
...
Процедура ОтключитьБлокировкуНачалаСеансовНаСервере(ИнформационнаяБаза, СтруктураПараметровБлокировки)
	...
	Если Не ЗначениеЗаполнено(СтруктураПараметровБлокировки) Тогда
		СтруктураПараметровБлокировки = АдминистрированиеЭлементовКластера.ПолучитьСтруктуруПараметорвБлокировкиБазы();
	КонецЕсли;
	
	СтруктураПараметровБлокировки.ВключенаБлокировкаНачалаСеансов = Ложь;
	АдминистрированиеЭлементовКластера.УстановитьБлокировкуНачалаСеансов(ИнформационнаяБаза, СтруктураПараметровБлокировки);
...
<<<<<<< HEAD
&НаКлиенте
Процедура ИдентификаторБазыНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	СкопироватьЗначениеВБуфер(Строка(Объект.ИдентификаторБазы));
	
КонецПроцедуры

=======
>>>>>>> 28310b53 (Иерархия хранилища)
