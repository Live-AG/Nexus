...
Процедура БлокировкаНачалаСеансовПриИзменении(Элемент)
	...
	Если БлокировкаНачалаСеансов Тогда
		...
		ОткрытьФорму("Справочник.ИнформационныеБазы.Форма.ФормаБлокировкиНачалаСеансов", 
						...
						РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		ОписаниеОбработчикаВопроса = Новый ОписаниеОповещения("ИзменениеБлокировкиНачалаСеансов", 
																		ЭтотОбъект, 
																		БлокировкаНачалаСеансов);
...
&НаКлиенте
Процедура ИзменениеБлокировкиНачалаСеансов(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		УстановитьБлокировкуРегламентныхЗаданийНаСервере(Объект.Ссылка, БлокировкаНачалаСеансов);
	Иначе
